...
const getMpesaAccessToken = async (baseUrl: string, consumerKey: string, consumerSecret: string) => {
  ...
  if (!response.ok || !payload?.access_token) {
    throw new Error(payload?.error_description || "Failed to get M-Pesa access token.");
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (!stkResponse.ok || !stkPayload?.CheckoutRequestID) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: stkPayload?.errorMessage || "M-Pesa request failed." }),
        { status: 500, headers: corsHeaders }
      );
    }
