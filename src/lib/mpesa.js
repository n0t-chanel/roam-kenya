...
export async function verifyMpesaReceipt(supabase, payload) {
  const invokeResult = await supabase.functions.invoke("mpesa-verify-receipt", {
    body: payload
  });
  ...
  const response = await fetch(`${supabaseUrl}/functions/v1/mpesa-verify-receipt`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      apikey: supabaseAnonKey,
