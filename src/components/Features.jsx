    ...
    <section className="py-24 bg-[#FDFCFB] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16">
          ...
          <p className="text-gray-500 font-light text-sm md:text-base max-w-sm border-l-2 border-gray-200 pl-6 pb-2">
            We don't just drive; we orchestrate seamless logistics so you can focus on what matters most.
          </p>
        </div>

        {/* Premium Light Bento Grid Layout */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {features.map((feature) => (
            <div 
              key={feature.id}
              className={`relative bg-white border border-gray-100 rounded-[2rem] p-8 md:p-10 overflow-hidden group hover:shadow-[0_20px_50px_rgba(0,0,0,0.05)] hover:border-gray-200 hover:-translate-y-1 transition-all duration-500 ${feature.span}`}
            >
              {/* Giant Watermark Number (Creates Depth) */}
              <div className="absolute -bottom-6 -right-4 text-[120px] font-black text-gray-50 group-hover:text-gray-100 transition-colors duration-500 leading-none select-none pointer-events-none font-serif">
                {feature.id}
              </div>

              {/* Icon Container */}
              <div className="w-16 h-16 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center mb-8 group-hover:bg-white group-hover:shadow-md transition-all duration-500">
                {feature.icon}
              </div>
              ...
              <div className="absolute top-0 left-0 w-0 h-[3px] bg-gradient-to-r from-[#C5A059] to-[#B35A38] group-hover:w-full transition-all duration-700 ease-out" />
            </div>
          ))}
        </div>
